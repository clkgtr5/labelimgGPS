...
#the following four functions are used to get the latitude & longitude
...
def _get_if_exist(data, key):
    """
    Returns the 'key' from the dictionary 'data' if exists.
    Otherwsie, it returns None
    """
    if key in data:
        return data[key]
    return None
...
def get_lat_lon(exif_data):
    """
    Returns the latitude and longitude, if available, from 
    the provided exif_data (obtained through get_exif_data above)
    """
    lat = None
    lon = None
